...
#load Seurat object with tree-based clusters
...
tmp <- readRDS("~/NeMO_analysis_folder/Transcriptomics/cross_species_integration/additional_intermediate_files/inh.clusters.finetuning.v2.rds")

#subset seurat object to only include cells that were clustered with tree-based method
...
cocl.subset <- cocl[grepl(compare.species[1], row.names(cocl)),
                    grepl(compare.species[3], row.names(cocl))] #change 3 to 2 for marmoset

row.names(cocl.subset) <- sub(paste0(compare.species[1], "_"), "",
                              row.names(cocl.subset))

colnames(cocl.subset) <- sub(paste0(compare.species[3], "_"), "", #change 3 to 2 for marmoset
...
marmoset_order <- readRDS("~/NeMO_analysis_folder/Transcriptomics/cross_species_integration/additional_intermediate_files/inh_marmoset_order.RDS")
mouse_order <- readRDS("~/NeMO_analysis_folder/Transcriptomics/cross_species_integration/additional_intermediate_files/inh_mouse_order.RDS")

#toggle between marmoset or mouse order depending on human/marm and human/mouse comparison
...
pheatmap(cocl.subset2, cluster_rows = FALSE, cluster_cols = FALSE,
         ...
         fontsize = 6, annotation_colors = ann_colors, cellwidth = 5, cellheight = 5, border_color = "grey90")
