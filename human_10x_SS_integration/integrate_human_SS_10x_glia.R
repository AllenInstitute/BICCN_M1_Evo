...
sample.1.combined <- subset(sample.1.combined, idents = c("glia"))
sample.2.combined <- subset(sample.2.combined, idents = c("glia"))
...
sample.combined <- FindClusters(sample.combined, resolution = 10, n.start = 100) 
...
sets = names(dat.list)
de.param.list = list(de_param(q1.th=0.4, q.diff.th=0.7, de.score.th=150,min.cells=10, min.genes = 4),  de_param(q1.th=0.5, q.diff.th=0.7, de.score.th=100,min.cells=2, min.genes = 1))
names(de.param.list) = sets
comb.dat$de.param.list = de.param.list
...
merge.cl =  merge_cl_multiple(comb.dat, comb.dat$dat.list, cl=cl.to.test, anchor.genes=merge.genes, verbose=TRUE)
